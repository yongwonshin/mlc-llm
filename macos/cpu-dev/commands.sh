...
# Build and run MLC LLM + TVM from source on macOS CPU.

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"

# 0) Ensure compiler toolchain is available.
xcode-select --install || true

# 1) Create and activate a dedicated conda environment.
ENV_NAME="${ENV_NAME:-mlc-llm-cpu-dev}"
...
if command -v llvm-config >/dev/null 2>&1; then
  ...
  if [ "${LLVM_CONFIG_MAJOR}" -ge 21 ]; then
    echo "Warning: LLVM ${LLVM_CONFIG_MAJOR} may break TVM builds; consider LLVM_VERSION=15." >&2
  fi
fi
...
BUILD_DIR="${BUILD_DIR:-${REPO_ROOT}/build/cpu-dev}"
...
if [ "${RUN_TESTS:-1}" = "1" ]; then
  TEST_SCRIPT="$(mktemp -t mlc-llm-cpu-dev-test.XXXXXX.py)"
  cat > "${TEST_SCRIPT}" <<PY
...
if [ "${RUN_CHAT:-0}" = "1" ]; then
  ...
  python -m mlc_llm chat "${MODEL}" --device cpu
else
