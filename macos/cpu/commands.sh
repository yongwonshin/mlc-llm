...
# Reproducible commands for running a public model with CPU on macOS.

cd "$(dirname "$0")"

# 1) Create and activate a dedicated conda environment.
ENV_NAME="${ENV_NAME:-mlc-llm-cpu}"
...
if [ "${SKIP_INSTALL:-0}" != "1" ]; then
  ...
  # 3) Install MLC LLM nightly (CPU wheel).
...
# 5) Run a public, small model on CPU (no token required).
